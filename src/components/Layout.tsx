...
const Navbar = () => {
  ...
  return (
    <nav aria-label="Main navigation" className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 px-6 py-4 flex justify-between items-center ${navBgClass}`}>
      <Link to="/" aria-label="Experience Andamans - Home" className="flex items-center gap-2">
        <img src="https://logo.clearbit.com/experienceandamans.com" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.style.display='none'; e.currentTarget.nextSibling.style.display='block'; }} alt="Experience Andamans Logo" className="h-8 md:h-12 object-contain" /><span className="text-2xl font-serif font-bold tracking-tighter uppercase transition-colors" style={{display: 'none'}}>Experience Andamans</span>
      </Link>
